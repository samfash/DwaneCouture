...
//   });
  ...
  it("should reset the user password using a valid reset token", async () => {
    ...
    const resetToken = result.rows[0].resettoken as string;
  ...
  it("should not allow OAuth users to log in with a password", async () => {
    await pool.query(
      "INSERT INTO users (email, password, role, isOAuth) VALUES ($1, $2, $3, $4)",
      ["oauthuser@example.com", "Secure@123", "user", true]
    );
