...
import { ProfileBase, MaleMeasurements, FemaleMeasurements } from '../../../modules/profile/profile.types';
...
const mockMaleMeasurements: MaleMeasurements = {
...
describe('Profile Service', () => {
  ...
  describe('handleProfileCreation', () => {
    it('should create a profile and measurements for male', async () => {
      ...
      const measurements: MaleMeasurements = {
        profile_id: 'profile1',
        neck: 15.5,
        chest: 40,
        waist: 32,
      ...
      (createProfile as jest.Mock).mockResolvedValue(createdProfile);
      (createMaleMeasurements as jest.Mock).mockResolvedValue(true);
      (getProfileWithMeasurements as jest.Mock).mockResolvedValue(profileWithMeasurements);
    ...
    it('should create a profile and measurements for female', async () => {
      ...
      const measurements: FemaleMeasurements = {
        profile_id: 'profile2',
        burst: 34,
        waist: 28,
        hips: 36,
      ...
      // Mock the return values of the functions
      (createProfile as jest.Mock).mockResolvedValue(createdProfile);
      (createFemaleMeasurements as jest.Mock).mockResolvedValue(true);
      (getProfileWithMeasurements as jest.Mock).mockResolvedValue(profileWithMeasurements);
    ...
    it('should throw an error if profile creation fails', async () => {
      ...
      (createProfile as jest.Mock).mockResolvedValue(null);
  ...
  describe('updateUserProfile', () => {
    ...
    it("should update profile and male measurements", async () => {
      ...
      (updateMaleMeasurements as jest.Mock).mockResolvedValue(undefined);
      (getProfileWithMeasurements as jest.Mock).mockResolvedValue({ profile: updatedProfile, measurements: mockMaleMeasurements });
    ...
    it("should update only measurements if profile is not provided", async () => {
      (getProfileWithMeasurements as jest.Mock).mockResolvedValue({ profile: mockProfile, measurements: mockMaleMeasurements });
    ...
    it("should not call updateMeasurement if measurement is not provided", async () => {
      const updatedProfile = { ...mockProfile, full_name: "Updated Name" };
      (updateProfile as jest.Mock).mockResolvedValue(updatedProfile);
      (getProfileWithMeasurements as jest.Mock).mockResolvedValue({ profile: updatedProfile, measurements: undefined });
