...
import { CreatePaymentInput } from "../../../modules/payments/payment.types";
...
describe("Payment Service", () => {
  ...
  it("should get a payment by order id", async () => {
    (paymentModel.getPaymentByOrderId as jest.Mock).mockResolvedValue(mockPayment);
  ...
  it("should throw error if payment not found", async () => {
    (paymentModel.getPaymentByOrderId as jest.Mock).mockResolvedValue(null);

    await expect(paymentService.getPaymentByOrderService("nonexistent-order")).rejects.toThrow("Payment not found");
  });

  it("should update payment status", async () => {
    (paymentModel.updatePaymentStatus as jest.Mock).mockResolvedValue(mockPayment);
