...
// Create profile
export const createProfile = async (profile: ProfileBase): Promise<ProfileBase> => {
  const query = `
...
export const createMaleMeasurements = async (profileId: string, measurements: MaleMeasurements) => {
...
export const createFemaleMeasurements = async (profileId: string, measurements: FemaleMeasurements) => {
  const keys = Object.keys(measurements);
  const values = Object.values(measurements);
...
export const getProfileWithMeasurements = async (userId: string) => {
...
// Update profile
export const updateProfile = async (profileId: string, updates: Partial<ProfileBase>) => {
...
export const updateMaleMeasurements = async (
  profileId: string,
  updates: Partial<MaleMeasurements>
): Promise<void> => {
  const keys = Object.keys(updates);
  const values = Object.values(updates);
...
export const updateFemaleMeasurements = async (
  profileId: string,
  updates: Partial<FemaleMeasurements>
): Promise<void> => {
  const keys = Object.keys(updates);
  const values = Object.values(updates);
  if (keys.length === 0) return;
