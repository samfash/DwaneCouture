...
//   useEffect(() => {
//   setMounted(true); // Prevent rendering until client-side
...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-[95%] max-w-6xl rounded-2xl backdrop-blur-md bg-background dark:bg-background border border-white/10 dark:border-white/20 shadow-lg px-6 py-3 flex justify-between items-center transition-all">
       {/* Logo */}
       <motion.div 
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.6 }}
        className="text-2xl font-extrabold text-foreground  dark:text-background"
      >
         <Link href="/">DwaneCouture</Link>
       </motion.div>
